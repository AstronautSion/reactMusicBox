...
import { StBtnLoginForm, StWarnningText } from "./styles";
...
const LoginForm = () => {
  ...
  const onSubmitFIndAccount = useCallback(
    (e) => {
      e.preventDefault();
      if (checkEmail) {
        dispatch(
  ...
  return (
    <>
      <form onSubmit={onSubmitFIndAccount}>
        ...
        />
        {!checkEmail && (
          <StWarnningText>이메일 형식을 다시 확인해 주세요.</StWarnningText>
        )}
        <StInput
          stMargin="5px 0 0 0"
          type="password"
        ...
        {!checkPassword && (
          <StWarnningText>
            비밀번를 확인해주세요 [대문자, 숫자, 특수문자를 포함한 최소 8 자]{" "}
          </StWarnningText>
        )}

        <StBtnLoginForm type="submit">YTList Login</StBtnLoginForm>
      </form>
      {loginError && <StWarnningText>{loginError}</StWarnningText>}
    </>
  );
};
